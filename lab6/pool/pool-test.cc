...
void test_run_in_order(int thread_count, int count, bool submit_first, bool wait_reverse) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        // std::cout << "done submitting" << std::endl;
    }
    // std::cout << "submit first before " << std::endl;
    if (submit_first) {
        pthread_barrier_wait(&barrier);
    }
    
    // std::cout << "submit first after " << std::endl;
    if (wait_reverse) {
        for (int i = count - 1; i >= 0; --i) {
            // std::cout << "Hanging on task: " << i << std::endl;
    ...
    // std::cout << destroy_counter.load() << std::endl;
